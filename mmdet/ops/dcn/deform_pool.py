...
class DeltaRPooling(DeformRoIPooling):
    ...
    def forward(self, data, rois, delta_r):
        assert data.size(1) == self.out_channels
        if self.no_trans:
            delta_r = data.new_empty(0)
            return deform_roi_pooling(data, rois, delta_r, self.spatial_scale,
                                      self.out_size, self.out_channels,
                                      self.no_trans, self.group_size,
                                      self.part_size, self.sample_per_part,
                                      self.trans_std)
...
class DeltaCPooling(DeformRoIPooling):
    ...
    def forward(self, data, rois, delta_c):
        assert data.size(1) == self.out_channels
        if self.no_trans:
            delta_c = data.new_empty(0)
            return deform_roi_pooling(data, rois, delta_c, self.spatial_scale,
                                      self.out_size, self.out_channels,
                                      self.no_trans, self.group_size,
                                      self.part_size, self.sample_per_part,
                                      self.trans_std)
